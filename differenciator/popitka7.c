...
#define dif(a) differ(a)
#define c(a) copy_tree(a)

#define CONST(n) create(number, 0, NULL, NULL)
#define VAR(n) create (number, 1, l(n), r(n))
#define PLUS(n1, n2) create (operation, '+', n1, n2)
#define SUB(n1, n2) create (operation, '-', n1, n2)
#define MUL(n1, n2) create (operation, '*', n1, n2)
#define DIV(n1, n2) create (operation, '/', n1, n2)
#define SIN(n) create (operation, 's', NULL, n)
#define COS(n) create (operation, 'c', NULL, n)
#define POW(n1, n2) create (operation, '^', n1, n2)
#define NUM(a) create (number, (a), NULL, NULL)

#define is_const(n) n->type == number
...
Node* create(int type, double val, Node* left, Node* right)
{
    Node* new_node = create_node(val, type);
    new_node->left  = left;
    ...
    switch (cur_el->type)
    {
        ...
        case operation:
        {
            switch ((int)cur_el->value)
            {
                case '+':
                {
                    new_node = PLUS(dif(l(cur_el)), dif(r(cur_el)));
                    break;
                }

                case '-':
                {
                    new_node = SUB(dif(l(cur_el)), dif(r(cur_el)));
                    break;
                }
                case '*':
                {
                    if (is_const (r(cur_el)) && is_const (l(cur_el)))
                        new_node = MUL(c(r(cur_el)), c(l(cur_el)));
                    else
                        new_node = PLUS(MUL(dif(l(cur_el)), c(r(cur_el))), MUL(dif(r(cur_el)), c(l(cur_el))));

                    break;
                }
                case 's':
                {
                    new_node = MUL(COS(r(cur_el)), dif(r(cur_el)));
                ...
                case 'c':
                {
                    new_node = MUL(NUM(-1), MUL(SIN(r(cur_el)), dif(r(cur_el))));

                    break;
                }
                case '/':
                {
                    new_node = DIV(SUB(MUL(dif(l(cur_el)), c(r(cur_el))), MUL(dif(r(cur_el)), c(l(cur_el)))), POW(c(r(cur_el)), NUM(2)));
                ...
                case '^':
                {
                    new_node = MUL(MUL(c(r(cur_el)), POW(c(l(cur_el)), NUM(r(cur_el)->value - 1))), dif(l(cur_el)));
                    break;
